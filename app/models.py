...
class Mark(db.Model):
    ...
    # component marks (nullable until uploaded)
    ...
    # derived values
    subject_score = db.Column(db.Float, nullable=True)  # 0-100 normalized
    risk = db.Column(db.String(10), nullable=True)      # 'low' / 'medium' / 'high'
    updated_on = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # 🔹 New canonical flags
    mid_fail = db.Column(db.Boolean, nullable=False, default=False)
    backlog = db.Column(db.Boolean, nullable=False, default=False)
    eligible_for_endsem = db.Column(db.Boolean, nullable=True)

    # 🔹 Link to UploadedFile
